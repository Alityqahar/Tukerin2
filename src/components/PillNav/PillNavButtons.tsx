...
export interface PillNavButtonsProps {
	items: PillNavItem[];
	activeHref?: string;
	className?: string;
	ease?: string;
...
const PillNavButtons: React.FC<PillNavButtonsProps> = ({
	items,
	activeHref,
	className = '',
	ease = 'power3.easeOut',
	...
	useEffect(() => {
		const layout = () => {
			circleRefs.current.forEach(circle => {
				...
				const label = pill.querySelector<HTMLElement>(`.${styles['pill-label']}`);
				const white = pill.querySelector<HTMLElement>(`.${styles['pill-label-hover']}`);
	...
	return (
		<div className={`${styles['pill-nav-container']} ${styles['pill-nav-right']}`}>
			<nav className={`${styles['pill-nav']} ${className}`} aria-label="Primary" style={cssVars}>
				{/* ...logo intentionally omitted - buttons/animations only... */}
								...
								{isRouterLink(item.href) ? (
									...
									>
										...
										<span className={styles['label-stack']}>
											...
											<span className={styles['pill-label-hover']} aria-hidden="true">
												{item.label}
											</span>
										</span>
									</Link>
			...
			<div className={`${styles['mobile-menu-popover']} ${styles['mobile-only']}`} ref={mobileMenuRef} style={cssVars}>
